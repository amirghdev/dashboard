...
export default {
    ...
    methods: {
        deleteItem(item) {
            this.$http.delete("Texts/delete", { data: { id: item.id, token: localStorage.getItem("token") } })
                .then(() => {
                    this.$toast.success(`${item.subject} حذف شد`);
                    this.$emit("getItems");
                }).catch((err) => {
        ...
        show(id) {
            this.$router.push({ name: "showText", params: { id } }).catch(() => { });
        }
    },
}
</script>
