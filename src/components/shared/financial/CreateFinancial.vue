<template>
    <article id="createFinancial" class="create" v-if="showPage">
        <h3 class="text-center mb-3">
            {{ type == "financial" ? "ساخت هزینه جدید" : "ساخت درامد جدید" }}
        </h3>
        <form @submit.prevent="createFinancial">
            <div class="form-group">
                <input type="text" name="subject" class="form-control" placeholder="عنوان را وارد کنید" ref="subject">
            </div>
            <div class="form-group">
...
export default {
    ...
    created() {
        ...
        if (this.$route.query.type != "financial" && this.$route.query.type != "income") {
            this.showPage = false;
            this.$router.push({ name: "financial" }).catch(() => { });
        }
        this.showPage = true;
    ...
    methods: {
        createFinancial() {
            ...
            this.$http.post("financial/createFinancial", { day: this.day, month: this.month, year: this.year, token: localStorage.getItem("token"), subject, price, type: this.type })
                .then(response => {
                    this.showErr = false;
